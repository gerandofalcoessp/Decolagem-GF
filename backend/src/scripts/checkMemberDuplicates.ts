...
async function checkMemberDuplicates() {
  try {
    ...
    const emailGroups = members.reduce((acc: any, member: any) => {
      if (!acc[member.email]) {
        acc[member.email] = [];
      }
    ...
    const duplicateEmails = Object.entries(emailGroups).filter(([_, members]: [string, any]) => members.length > 1);

    if (duplicateEmails.length > 0) {
      console.log('⚠️  Duplicatas encontradas por email:');
      duplicateEmails.forEach(([email, members]: [string, any]) => {
        console.log(`\n📧 Email: ${email}`);
    ...
    const duplicateAuthUsers = Object.entries(authUserGroups).filter(([_, members]: [string, any]) => members.length > 1);

    if (duplicateAuthUsers.length > 0) {
      console.log('\n⚠️  Duplicatas encontradas por auth_user_id:');
      duplicateAuthUsers.forEach(([authUserId, members]: [string, any]) => {
    ...
    if (membersWithoutAuth.length > 0) {
      ...
      membersWithoutAuth.forEach((member: any) => {
