...
let regionalsCache: Regional[] | null = null;
 ...
 */
async function getRegionals(): Promise<Regional[]> {
  const now = Date.now();
 ...
 */
export async function getUserRegionalId(userRegional: string): Promise<string | null> {
  try {
 ...
 */
export async function getEventRegionalId(eventRegional: string): Promise<string | null> {
  try {
    const regionals = await getRegionals();
    
    // Mapeamento de nomes de eventos para nomes de regionais
    const eventToRegionalMapping: { [key: string]: string } = {
      'norte': 'Norte',
 ...
 */
export async function canUserSeeRegionalEvents(userRegional: string, eventRegional: string): Promise<boolean> {
  try {
 ...
 */
export function clearRegionalsCache(): void {
  regionalsCache = null;
  cacheTimestamp = 0;
}