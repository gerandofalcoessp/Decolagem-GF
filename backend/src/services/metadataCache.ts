...
class MetadataCache {
  ...
  private columnCache = new Map<string, ColumnMetadata>();
  private readonly CACHE_TTL = 5 * 60 * 1000; // 5 minutos em produção
  private readonly DEV_CACHE_TTL = 30 * 1000; // 30 segundos em desenvolvimento

  private getCacheTTL(): number {
    return process.env.NODE_ENV === 'production' ? this.CACHE_TTL : this.DEV_CACHE_TTL;
  }

  private isExpired(lastChecked: number): boolean {
   ...
   */
  getCacheStats(): { tables: number; columns: number; ttl: number } {
    return {
   ...
   */
  async preloadMainTables(): Promise<void> {
    const mainTables = [
      'regional_activities',
      'instituicoes', 
      'goals',
