 ...
 * Atualiza um usuário
 */
router.put('/users/:id', authMiddleware, requireRole('super_admin'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (nome || regional || role || tipo || funcao) {
            ...
            if (nome)
                userData.user_metadata.nome = nome;
            if (regional)
                userData.user_metadata.regional = regional;
            if (role)
                userData.user_metadata.role = role;
            if (tipo)
                userData.user_metadata.tipo = tipo;
            if (funcao)
                userData.user_metadata.funcao = funcao;
        }
        ...
        try {
            ...
            if (existingUsuario) {
                ...
                if (nome)
                    usuarioUpdateData.nome = nome;
                if (email)
                    usuarioUpdateData.email = email;
                if (funcao)
                    usuarioUpdateData.funcao = funcao;
                if (regional)
                    usuarioUpdateData.regional = regional;
                if (role) {
 ...
 * Bloqueia um usuário
 */
router.post('/users/:id/block', authMiddleware, requireRole('super_admin'), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post('/users/:id/unblock', authMiddleware, requireRole('super_admin'), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.put('/users/:id/update-password', authMiddleware, requireRole('super_admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { newPassword } = req.body;
        if (!newPassword) {
 ...
 */
router.post('/users/:id/generate-password', authMiddleware, requireRole('super_admin'), async (req, res) => {
    try {
        const { id } = req.params;
